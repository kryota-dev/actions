...
- **Renovate compatibility**: Renovate's `github-actions` manager automatically discovers `action.yml` files matching `/(^|/)action\.ya?ml$/`, so `.github/composite/` files are covered by default.
...
1. **Directory structure**: Store Composite Actions under `.github/composite/{action-name}/action.yml` (kebab-case naming enforced by ls-lint)
2. **External reference format**: `kryota-dev/actions/.github/composite/{action-name}@{tag-or-sha}`
...
5. **ls-lint configuration fix**: Correct the top-level key from `ls-lint:` to `ls:` and add `.github/composite/` rules (`.dir: kebab-case`, `.yml: regex:action`)
6. **No changes required** for: zizmor (auto-discovers `action.yml`), Renovate (auto-discovers `action.yml`), tagpr/release management (tags apply repository-wide), actionlint (naturally excludes `.github/composite/`)

## Consequences

- Composite Actions are stored under `.github/composite/` with kebab-case directory names
...
- External repositories can reference Composite Actions via `kryota-dev/actions/.github/composite/{action-name}@vX`
