...
> ソースファイル: [`.github/actions/deploy-rsync/action.yml`](../deploy-rsync/action.yml)

rsync over SSH を使用してアーティファクトをサーバーにデプロイする Composite Action です。dry-run モードと本番モードをサポートし、SSH 秘密鍵のクリーンアップも自動実行します。
...
```yaml
steps:
  - uses: kryota-dev/actions/.github/actions/deploy-rsync@v1
    with:
