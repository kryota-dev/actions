...
> ソースファイル: [`.github/actions/deploy-ftp/action.yml`](../deploy-ftp/action.yml)

lftp を使用して FTP サーバーにアーティファクトをデプロイする Composite Action です。dry-run モードと本番モードをサポートしています。
...
```yaml
steps:
  - uses: kryota-dev/actions/.github/actions/deploy-ftp@v1
    with:
