...
> ソースファイル: [`.github/actions/delete-rsync/action.yml`](../delete-rsync/action.yml)

rsync over SSH を使用してリモートサーバー上のディレクトリを削除する Composite Action です。空ディレクトリとの同期による削除方式で、dry-run モードをサポートし、SSH 秘密鍵のクリーンアップも自動実行します。
...
```yaml
steps:
  - uses: kryota-dev/actions/.github/actions/delete-rsync@v1
    with:
